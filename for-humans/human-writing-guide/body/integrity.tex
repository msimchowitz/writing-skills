...
The examples below are schematic. Their theorem conditions, task names, and
numbers are invented; each pair isolates one editing choice.
...
\begin{quote}
\textbf{Overstated:} Theorem~3 shows that action chunking prevents compounding
error.

\textbf{Theorem~3 establishes:} Under open-loop stability and bounded disturbances,
Theorem~3 bounds rollout error for horizons up to $H$.
\end{quote}

\subsection{Separate hypotheses, planned tests, and findings}

Write ``we test whether'' when the paper poses a question, ``we hypothesize''
when it proposes an explanation, and ``we find'' only after the evidence
supports a result. ``We believe'' and ``a natural follow-up'' do not tell the
reader the statement's status as a hypothesis, planned test, or finding.

Make the source of a claimed capability explicit. Suppose a visual backbone
is frozen during adaptation. Robustness to visual perturbations must then come
primarily from the pretrained representation, but that reasoning is still a
hypothesis until the evaluation tests it. A scoped statement is:

\begin{quote}
Because the visual backbone is frozen during adaptation, robustness to visual
perturbations must primarily come from the pretrained representation. We test
whether that robustness persists over the perturbation range considered here.
\end{quote}

Name the environment, perturbation, task mode, or resource constraint that
limits each result. Do not promote a planned comparison, an expected
qualitative outcome, or an intuition from another field into a validated
claim.

\subsection{Check every promised contribution against the evidence}
...
\begin{quote}
\textbf{Unsupported:} Method A consistently outperforms prior methods
across diverse environments.

\textbf{Supported by Figure~2:} At 100{,}000 interactions, Method A
...
\begin{quote}
\textbf{Missing comparison details:} Method A improves success by eight
percentage points.

\textbf{With comparison details:} Compared with Method B on Task Q, Method A raises
...
\subsection{State limits where the result first appears}

A limitation is useful when it changes the result's application or
interpretation. Name the condition that creates the limit and state the
remaining support. If several limits matter, explain each one separately.
...
\begin{quote}
\textbf{Qualified too late:} The method is robust to distribution shift. The
limitations section later reveals that the tests changed only image texture
in simulation.

\textbf{Qualified up front:} The method maintains its success rate under the
