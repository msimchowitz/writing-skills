\section{Keep the LaTeX source easy to inspect}
\label{sec:latex}

The source tree can make revision easy or cumbersome. A clear project prevents
duplicated text and accidental notation changes. It also lets several authors
work without breaking a submission mode.

\subsection{Use the entry point as an outline}

Keep the top-level entry point short enough to read as the manuscript's table
of contents. It should show the build mode and manuscript order at a glance.
Put prose in section files:
...
\subsection{Use each toggle for one build distinction}
...
Keep toggles near the top of the entry point. Build every supported mode before
release. The public and anonymous versions may look different, but they should
make the same scientific claims.
