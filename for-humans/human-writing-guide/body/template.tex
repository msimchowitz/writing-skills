...
\path{main.tex}. Then remove unused definitions from
...
\subsection{Know which file owns each choice}

The include order lives in \path{preamble/_preamble_includes.tex}. Keep that
order until there is a specific reason to change it. The files have different
jobs:
...
narrowest file that owns it. A task name does not belong in
\texttt{header.tex}. A package import does not belong in
\texttt{specific\_macros.tex}.

\subsection{Use the theorem environments that are actually defined}

With \texttt{customthms} enabled, choose an environment by its role:
...
\subsection{Choose build toggles by responsibility}

The toggles appear near the top of \texttt{main.tex}:
...
\texttt{toreturn} toggle controls whether return notes are visible; those
...
change in role: setup, result, mechanism, boundary, or implication.
...
\texttt{color\_defs.tex} centralizes four different jobs. Link and citation
...
when a new paper carries all of those decisions without knowing which ones it
still needs.
